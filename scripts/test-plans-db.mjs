...
try {
  ...
  const plan = plans.createPlan(1, { term: '1930', name: 'Ago–Dic 2026' });
  planDetailSchema.parse(plan);
  assert.equal(plan.items.length, 0);
  assert.throws(() => plans.createPlan(1, { term: '', name: 'x' }), /término y nombre/);

  // Materia sin grupo: entra como 'desired'.
  let detail = plans.addPlanItem(1, plan.id, { courseId: 1, note: 'con Pérez si abre' });
  ...
  assert.throws(() => plans.addPlanItem(1, plan.id, { courseId: 1 }), /ya está en el plan/);
  ...
  detail = plans.updatePlanItem(1, plan.id, itemId, { sectionId: 10 });
  ...
  assert.throws(() => plans.updatePlanItem(1, plan.id, itemId, { sectionId: 12 }), /otra materia/);
  assert.throws(() => plans.updatePlanItem(1, plan.id, itemId, { sectionId: 13 }), /otro término/);

  // Quitar la sección la vuelve 'desired'; tocar solo el candado no la toca.
  detail = plans.updatePlanItem(1, plan.id, itemId, { locked: true });
  assert.equal(detail.items[0].locked, true);
  assert.equal(detail.items[0].status, 'planned', 'el candado no toca la sección');
  detail = plans.updatePlanItem(1, plan.id, itemId, { sectionId: null });
  assert.equal(detail.items[0].status, 'desired');
  assert.equal(detail.items[0].section, null);

  // Resumen con créditos sumados.
  plans.updatePlanItem(1, plan.id, itemId, { sectionId: 11 });
  plans.addPlanItem(1, plan.id, { courseId: 2 });
  const [summary] = plans.listPlans(1);
  ...
  const recommended = plans.createPlanWithItems(1, {
    term: '1930',
  ...
  const beforeInvalid = plans.listPlans(1).length;
  assert.throws(
    () => plans.createPlanWithItems(1, {
      term: '1930', name: 'Inválido', items: [{ courseId: 1, sectionId: 13 }],
    }),
    /otro término/
  );
  assert.equal(plans.listPlans(1).length, beforeInvalid, 'una propuesta inválida no deja plan parcial');

  // Duplicar copia items con sección, nota y candado.
  const copy = plans.duplicatePlan(1, plan.id);
