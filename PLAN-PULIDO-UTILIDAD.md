...
- **Estado:** P0–P6 cerradas. El gate de utilidad está completo; la publicación
  del primer tag sigue esperando la revisión del operador, no al plan.
...
| P3 — Inicio y calendario | ✅ Hecho | Adaptador read-only del calendario académico oficial (`src/academicCalendar.js` + parser puro `src/shared/academicCalendar.ts`), leyendo los bloques `application/ld+json` de schema.org/Event que publica pucmm.edu.do — datos estructurados, no HTML de presentación. Caché en SQLite (migración 6, tabla compartida sin `user_id` porque el dato es institucional) con id oficial, título, rango, enlace, página de origen y fecha de fetch. Dedupe por `@id` y, de respaldo, por título normalizado + rango: el calendario real publica «Último día de docencia» dos veces el mismo día con ids distintos. Fechas parseadas a mano desde `2026-8-10` (PUCMM no zero-padea) y tratadas como texto `YYYY-MM-DD` en `America/Santo_Domingo`, sin pasar nunca por `new Date(iso)`, que las correría un día. TTL de 24h, `needsPortal: false` — es público, así que sigue funcionando sin sesión y no cede ante una inscripción. Un fallo nunca vacía la caché ni bloquea Inicio; un 200 sin eventos se registra como error visible en diagnostics. **Decisión:** el calendario de preinscripción se enlaza y no se parsea — el recon confirmó que no publica JSON-LD y sus horas son por facultad, no personales. Inicio: fechas institucionales tras la agenda del día, holds solo cuando son accionables, y aula promovida a línea propia por encima de código y NRC. Smoke real contra la página pública: 41 fechas. Tests: `test-academic-calendar` (formato de fecha, timezone, dedupe real, offline, markup cambiado, idempotencia) + suite, typecheck y lint en verde. |
| P4 — Horario enriquecido | ✅ Hecho | Merge por campo y procedencia en `src/shared/sectionMerge.ts`, aplicado en `saveSection`: un valor vacío, un `TBA` o un conjunto de encuentros más pobre nunca reemplazan uno más rico. El bug concreto que cierra: `instructor = excluded.instructor` pisaba incondicionalmente y View My Classes manda `instructor: null`, así que **cada sync de horario borraba el profesor** que Class Search había enriquecido. Autoridad por campo — horario y aula de View My Classes, profesor y metadatos de Class Search — con `instructor_source` y `meetings_source` persistidos (migración 8) y discrepancias escritas a diagnostics en vez de resolverse calladas. Correlación por STRM + class number (la `UNIQUE(term, class_nbr)` de la tabla), nunca por código o sección: probado que dos grupos parecidos no intercambian profesor ni aula y que el mismo NRC en otro ciclo es otra sección. No hizo falta scraper nuevo. UI: lista reordenada a materia → hora → **aula con icono y peso propio** → profesor o «Profesor no publicado» → código y NRC como metadatos; cada fila y cada bloque de la grilla abren `ClassDetail`, un diálogo real con foco, Escape y `aria-modal` que reemplaza al `title=` nativo — toda la información del hover existe ahora en teclado y touch. Lista por defecto en móvil, grid en desktop, `@page size: landscape` e ICS ya existentes y conservados. Tests: `test-section-merge` (vacío/TBA/riqueza, autoridad, conflictos, no-mezcla entre grupos y entre ciclos) + suite, typecheck y lint en verde. |
| P5 — Notas y señales | ✅ Hecho | Acumulado oficial de PeopleSoft **persistido** (migración 9, `gpa_official`) — antes se leía en cada sync y se tiraba, sobreviviendo solo como texto en un mensaje de error. `src/shared/projection.ts` lo usa de baseline y deja el cálculo por materias como auditoría: si difieren más que la precisión oficial (0.05, porque el portal publica 2.800 donde 402/143 da 2.8112) la proyección **se suspende** y se muestra la diferencia con su causa probable, en vez de elegir en silencio una política de repetidas. Dos horizontes separados con `creditsInProgressFor(courses, term)`: «Al cerrar este ciclo» solo cuenta créditos en curso **de ese ciclo** —sumar los del cuatrimestre siguiente era lo que inflaba el mejor caso— y «Hasta graduarte» los pendientes del advisement. Cada uno muestra baseline, créditos, promedio asumido, resultado a dos decimales y cómo lo redondearía PeopleSoft, etiquetado como escenario; «Cómo se calcula» expone la fórmula y ambos acumulados. Gráfica: el sparkline de 132×34 se sustituye por `TrajectoryChart` en SVG propio (sin librería nueva, decisión 7) con escala fija 0–4, líneas de referencia, acumulado por ciclo como serie principal, índice del período como secundaria opcional, blancos táctiles de ≥14px, `tabIndex` por punto y detalle idéntico en hover, foco y tap más resumen `sr-only`. Señales: `gpa-trend` se parte en `recent-change` (último ciclo contra el anterior) y `rolling-trend` (tres ciclos), así una caída reciente ya no puede aparecer como «subiendo»; cada señal lleva `severity`/`recency`/`actionability`/`confidence` y el orden sale de esos metadatos, no del orden del array. Una señal principal a ancho completo bajo «Lo más importante», el resto bajo «Contexto», y el color de alarma reservado a `severity: risk`. Tests: `test-projection` (reconciliación, acotamiento de créditos, punto fijo de mantener, suspensión) e `test-insights` actualizado + suite (72), typecheck y lint en verde. |
| P6 — Gate de lanzamiento | ✅ Hecho | `test-journeys` recorre los siete escenarios del plan juntos contra una base desechable, con fixtures sintéticos: etiqueta sin STRM que después lo descubre sin duplicarse, día con clases y día vacío, ciclo próximo con y sin período publicado, historial con repetida y acumulados que coinciden y que discrepan, calendario fresco/offline/markup roto, y carrito relevante con cinco componentes pidiéndolo a la vez. `npm run smoke` volvió a correr: **llevaba fallando desde `7131332`** —la aserción de ⌘K esperaba una página de secciones que ese commit eliminó— y además dependía del estado local de la máquina, por lo que en un checkout limpio moría en onboarding y en uno configurado capturaba historial académico real. Ahora sesión, onboarding, estado y catálogo son fixtures sintéticos y cubre 7 rutas × 390/768/1440 px sin desbordes, ⌘K operable sin mouse (Enter abre detalle, Esc cierra), PWA e impresión apaisada. Accesibilidad: `prefers-reduced-motion` respetado globalmente, foco visible por sistema, objetivos táctiles de 44px, y el `title=` nativo del horario reemplazado por un diálogo con foco y Escape. Smoke local **read-only** contra el portal real con la cuenta del operador: onboarding, login, las 9 rutas y los redirects, sin acciones mutantes — el calendario oficial trajo 41 fechas vivas y el orquestador cerró en «todo al día». `npm test` (75), `npm run typecheck`, `npm run lint` y `npm run audit:public` en verde. |
